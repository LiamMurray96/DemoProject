import pygame
import sys

larghezza, altezza = 800, 600

# pygame setup
pygame.init()

font = pygame.font.Font(None, 36)

def main_menu():
#creazione della schermata iniziale
text_start = font.render('inizia', True, "white")
text_instrucions = font.render('instruzioni', True, "white")
text_exit = font.render('Esci', True, "white")

#creiamo pulsante
start_button = pygame.Rect(150, 200, 200, 50)
instrucions_button = pygame.Rect(270, 1, 1, 1)
exit_button = pygame.Rect(1, 1, 1, 1)


screen = pygame.display.set_mode((larghezza, altezza))
clock = pygame.time.Clock()

icon = pygame.image.load("img/pacman.png")
pygame.display.set_icon(icon)

running = True
dt = 0

player_pos = pygame.Vector2(screen.get_width() / 2, screen.get_height() / 2)
def start_game():
    while running:
        # poll for events
        # pygame.QUIT event means the user clicked X to close your window
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False

        # fill the screen with a color to wipe away anything from last frame
        screen.fill("purple")

        #creazione rettangoli
        pygame.draw.rect(screen, "cyan", start_button)
        pygame.draw.rect(screen, "cyan", instrucions_button)
        pygame.draw.rect(screen, "cyan", exit_button)

        screen.blit(text_start, "blue")

        mouse_pos = 
        if start_button.collidepoint(mouse_pos) and (mouse_click[0])
            pygame.quit()
            sys.exit()

        pygame.draw.circle(screen, "red", player_pos, 40)

        keys = pygame.key.get_pressed()
        if keys[pygame.K_w]:
            player_pos.y -= 300 * dt
        if keys[pygame.K_s]:
            player_pos.y += 300 * dt
        if keys[pygame.K_a]:
            player_pos.x -= 300 * dt
        if keys[pygame.K_d]:
            player_pos.x += 300 * dt

        # flip() the display to put your work on screen
        pygame.display.flip()

        # limits FPS to 60
        # dt is delta time in seconds since last frame, used for framerate-
        # independent physics.
        dt = clock.tick(60) / 1000

pygame.quit()